      ...
      for radius in AK8 AK15 CA15; do
         for bkg in Higgs QCD; do
            for name_variable in gen_ norm ; do
               ...
               python code/preProcessing_Lola_variables.py $file_min $[$file_min+$a] $pt_min[${index}] $pt_max[${index}] $name_variable $bkg $radius $step $flag_merge &
               sleep 0.1
            done
         done
      done
